import './About.css';

function About() {
    const teamMembers = [
        {
            name: "Emma Spear",
            github: "https://github.com/elspear",
            linkedin: "https://www.linkedin.com/in/emma-spear-dev/",
            portfolio: "https://www.emmaspear.dev/"
        },
        {
            name: "Tammy Healy",
            github: "https://github.com/TimTam5566",
            linkedin: "https://www.linkedin.com/in/tamala-healy/",
            portfolio: "https://tamalahealy.netlify.app/"
        },
        {
            name: "Krista Soosaar",
            github: "https://github.com/kristasoos",
            linkedin: "https://www.linkedin.com/in/krista-soosaar-1850621a7/",
            portfolio: "https://kristasoos.github.io/index.html"
        },
        {
            name: "Juliane Gutierrez",
            github: null,
            linkedin: "https://www.linkedin.com/in/juliane-gutierrez-41718252/",
            portfolio: null
        },
        {
            name: "Jinfeng Shan",
            linkedin: "https://www.linkedin.com/in/jinfeng-lily-shan-54a59032/",
            portfolio: null
        }
    ];

    return (
        <div className='about-page-container'>
            {/* Hero Section */}
            <section className='about-hero-section'>
                <h1 className='about-hero-title'>About Save Point</h1>
                <p className='about-hero-subtitle'>The story behind the app and the team that built it</p>
            </section>

            {/* Project Story Section */}
            <section className='project-story-section'>
                <div className='story-card'>
                    <h2 className='story-title'>Our Story</h2>
                    <p className='story-text'>
                        Save Point was created as our final group project for the <strong>She Codes Plus Perth 2025 Cohort</strong>. 
                        We set out to build a tool that makes team retrospectives more engaging, collaborative, and fun.
                    </p>
                    <p className='story-text'>
                        Because your team deserves more than a wall of sad Post-it notes. 💜
                    </p>
                </div>
            </section>

            {/* Tech Stack Section */}
            <section className='tech-stack-section'>
                <h2 className='section-title'>Built With</h2>
                <div className='tech-grid'>
                    <div className='tech-item'>
                        <span className='tech-name'>React</span>
                    </div>
                    <div className='tech-item'>
                        <span className='tech-name'>Django</span>
                    </div>
                    <div className='tech-item'>
                        <span className='tech-name'>WebSockets</span>
                    </div>
                    <div className='tech-item'>
                        <span className='tech-name'>PostgreSQL</span>
                    </div>
                    <div className='tech-item'>
                        <span className='tech-name'>Netlify</span>
                    </div>
                    <div className='tech-item'>
                        <span className='tech-name'>Heroku</span>
                    </div>
                </div>
            </section>

            {/* Team Section */}
            <section className='team-section'>
                <h2 className='section-title'>Meet the Purrgrammers</h2>
                <div className='team-grid'>
                    {teamMembers.map((member, index) => (
                        <div key={index} className='team-member-card'>
                            <h3 className='member-name'>{member.name}</h3>
                            <div className='member-links'>
                                {member.linkedin && (
                                    <a 
                                        href={member.linkedin} 
                                        target='_blank' 
                                        rel='noopener noreferrer'
                                        className='social-link linkedin'
                                        title='LinkedIn'
                                    >
                                        <svg viewBox="0 0 24 24" fill="currentColor" className="social-icon">
                                            <path d="M20.447 20.452h-3.554v-5.569c0-1.328-.027-3.037-1.852-3.037-1.853 0-2.136 1.445-2.136 2.939v5.667H9.351V9h3.414v1.561h.046c.477-.9 1.637-1.85 3.37-1.85 3.601 0 4.267 2.37 4.267 5.455v6.286zM5.337 7.433c-1.144 0-2.063-.926-2.063-2.065 0-1.138.92-2.063 2.063-2.063 1.14 0 2.064.925 2.064 2.063 0 1.139-.925 2.065-2.064 2.065zm1.782 13.019H3.555V9h3.564v11.452zM22.225 0H1.771C.792 0 0 .774 0 1.729v20.542C0 23.227.792 24 1.771 24h20.451C23.2 24 24 23.227 24 22.271V1.729C24 .774 23.2 0 22.222 0h.003z"/>
                                        </svg>
                                    </a>
                                )}
                                {member.github && (
                                    <a 
                                        href={member.github} 
                                        target='_blank' 
                                        rel='noopener noreferrer'
                                        className='social-link github'
                                        title='GitHub'
                                    >
                                        <svg viewBox="0 0 24 24" fill="currentColor" className="social-icon">
                                            <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/>
                                        </svg>
                                    </a>
                                )}
                                {member.portfolio && (
                                    <a 
                                        href={member.portfolio} 
                                        target='_blank' 
                                        rel='noopener noreferrer'
                                        className='social-link portfolio'
                                        title='Portfolio'
                                    >
                                        <span className='material-icons'>language</span>
                                    </a>
                                )}
                            </div>
                        </div>
                    ))}
                </div>
            </section>

            {/* Footer */}
            <section className='about-footer-section'>
                <p className='about-footer-text'>Made with 💜 by Purrgrammers</p>
                <p className='about-footer-subtext'>She Codes Plus Perth 2025</p>
            </section>
        </div>
    );
}

export default About;